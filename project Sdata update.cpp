...
struct Queue {
    ...
    void insertFront(int x) { // untuk UNDO
...
Node* deleteTree(Node* node, int id) {
    if (!node) return NULL;

    if (id < node->id)
        node->left = deleteTree(node->left, id);
    else if (id > node->id)
        node->right = deleteTree(node->right, id);
    else {
        // ketemu node
        if (!node->left) {
        ...
        node->id = temp->id;
        node->right = deleteTree(node->right, temp->id);
    }
    return node;
}
    ...
    do {
        ...
        if (menu == 1) {
            ...
            if (undoStack.kosong()) {
                ...
                // HAPUS DARI TREE (RIWAYAT)
                root = deleteTree(root, id);
